...
export function levelMark(standing, size = 22, opts = {}) {
  ...
  svg.innerHTML =
    ...
    '<circle class="lvl-arc" cx="50" cy="50" r="' + r + '" stroke-dasharray="' + dash(solid)
      + '" transform="rotate(-90 50 50)"/>' +
    // Three digits in the same ring as one would be a smudge; the number gives way, not the ring.
    '<text class="lvl-n" x="50" y="50" text-anchor="middle" dominant-baseline="central" font-size="'
      + (String(standing.level).length >= 3 ? 30 : String(standing.level).length === 2 ? 38 : 44) + '">' + standing.level + "</text>";
