...
| A habit's name and unit are effective-dated in `history`; the read-model uses the unit in force per day and never reduces across a unit change | `js/habits.js` — HABIT_DEF `history`, `unitOn`/`nameOn`, `js/dailyfacts.js` — `forHabit` unit-scoping | `test/change-events.test.mjs` |
| `habit_exclude` flags a member/habit out of ANALYSIS (latest wins, follows merges); the read-model honours it, the board/history do not | `js/habits.js` — EXCLUDE reducer + `isExcluded`, `js/store.js` — `setExcluded`, `js/dailyfacts.js` filter | `test/change-events.test.mjs` |
| Every log carries provenance (`entryMethod`: typed/sensor/meter), stamped at the write boundary and derived for old events; the read-model weights it by confidence | `js/schema.js` — `ENTRY_METHOD`/`ev.log`, `js/habits.js` — `methodOn` + LOG reducer, `js/dailyfacts.js` — `method`/`confidence` | `test/provenance.test.mjs` |
| A malformed event is refused at the write boundary and skipped on replay (poison-event defence) | `js/schema.js` — `validate`, `js/store.js` — `commit`/`commitAll`, `js/habits.js` — replay skip | `test/provenance.test.mjs` |
| Insights facts 'Worth noticing' and 'A day to watch' are drawn from the read-model (any habit pair, per-habit weekday), floored so they appear only with enough data | `js/facts.js` — `correlation`/`weekdayWatch` over `js/dailyfacts.js` | `test/facts-patterns.test.mjs` |
| A `habit_member_merge` folds one member id into another; replay resolves it (union-find) at the single memberId read, so logs/streaks/board/read-model unify at once | `js/habits.js` — pre-scan + `canonicalMember`/`aliasesOf`, `js/store.js` — `mergeMember`, `js/ui/habitsheet.js` — merge control | `test/merge.test.mjs` |
...
| A workout done is a workout kept: there is no removal, the log is append-only, and a finish that was a test is shown for what it was | `js/habits.js` — `T.WORKOUT` replay; `js/workout.js` — `exerciseHistory` (zero sets are skipped sets) | `test/workout.test.mjs` |
